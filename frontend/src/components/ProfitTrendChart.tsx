...
import type { MonthlyFinancialTrend } from '../api/forecasting';
...
export default function ProfitTrendChart({ data }: Props) {
  ...
  useEffect(() => {
    ...
    // Y scale
    const maxValue = d3.max(data, d => Math.max(d.revenue, d.cost, d.profit)) || 0;
    const y = d3.scaleLinear()
      .domain([0, maxValue * 1.1])
      .range([height, 0]);

    // Line generators
    const revenueLine = d3.line<MonthlyFinancialTrend>()
      .x(d => (x(d.month) || 0) + x.bandwidth() / 2)
      .y(d => y(d.revenue))
      .curve(d3.curveMonotoneX);

    const profitLine = d3.line<MonthlyFinancialTrend>()
      .x(d => (x(d.month) || 0) + x.bandwidth() / 2)
      .y(d => y(d.profit))
      .curve(d3.curveMonotoneX);
