...
export default function BudgetCategoryChart({ data }: Props) {
  ...
  useEffect(() => {
    ...
    // Subgroups
    const subgroups = ['budgeted', 'actual'];
    ...
    svg.append('g')
      ...
      .selectAll('rect')
      .data(d => subgroups.map(key => ({ key, value: key === 'budgeted' ? d.budgeted : d.actual })))
      .join('rect')
